...
**Current phase: Phase 12 — Home Assistant Module.** 14 modules. ~64 commits, 263+ tests. New Home Assistant module: polls entity states every 5m, exposes REST API for status/config/service calls, dashboard page with entity cards grouped by domain, On/Off/Activate buttons for lights/switches/scenes. S7 dashboard admin expansion complete (SSE live toasts, per-module force-poll, document content editor, bulk source_type, sidebar alert badges). Kanban module polished (3 bugfixes, 42 inline styles extracted, 56 tests). Notflix frontend fixed (per-service stats rendering). Kanban boards with agent task orchestration (backported from LlamaBan), user identity profiles with API key management, MCP server (22 JSON-RPC tools), agent mailboxes (inbox/sent/thread/read), caching layer (in-memory, TTL + tag invalidation), per-module settings (settings.json overlay + dashboard forms), dashboard admin expansion (sidebar categories, document management table, module health, mobile responsive, theme toggle, command palette).
